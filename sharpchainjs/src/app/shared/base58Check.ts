...
export function getBytes(base58String: string): Buffer {
    ...
    if (!validateChecksum(allBytes)) throw new Error('invalid buffer checksum')
...
function getBytesInternal(base58String: string): Buffer {
    ...
    const finalBuffer = Buffer.concat([preBuffer, allDatabuffer])
    return finalBuffer
}
...
function validateChecksum(buffer: Buffer): boolean {
    ...
    const compareResult = Buffer.compare(expectedChecksum, actualChecksum)
